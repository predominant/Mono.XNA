...
#include <fbxfilesdk/fbxfilesdk_nsbegin.h>
	...
	************************************************************************/
	...
	public: 
		...
		// Start and Stop time. Merge in this interval.
		KTime mStart;
		KTime mStop;
	...
	* \param pTangentThreshold	Threshold used when comparing tangents. If the difference is over the threashold, we break the tangent.
	*/
...
KFBX_DLL void KFCURVE_ComputeTCBFromBezierForKey( double &pT, double &pC ,double &pB , double pLTan, double pRTan, HKFCurve pCurve, int pIndex );
	...
	* \param pFCurveNode					FCurveNode to modify.
	* \param pReferenceFCurveNode			FCurveNode we use as the reference.
	* \param pFidelityThreshold			Threshold used when comparing values. If the difference is over the threashold, we add a new key.
	...
	* \param pInterpolation	The interpolation type we are quering for.
	...
	* \param pDstNode		The Destination KFCurveNode in wich all layers will be merged.
