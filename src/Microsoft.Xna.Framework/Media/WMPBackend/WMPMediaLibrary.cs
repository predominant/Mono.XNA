...
namespace Microsoft.Xna.Framework.Media
{
    internal class WMPMediaLibrary : IMediaLibrary
    {
        ...
        {
            WMPLib.IWMPMediaCollection media = wmp.mediaCollection;
            WMPLib.IWMPPlaylistCollection playlistcollection = wmp.playlistCollection;
            WMPLib.IWMPPlaylist playlist = media.getAll();
            List<Song> songlist = new List<Song>();

            for (int i = 0; i < playlist.count; i++)
            {
                WMPLib.IWMPMedia temp = playlist.get_Item(i);
